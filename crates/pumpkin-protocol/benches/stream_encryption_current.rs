...
/// Reproduces the pre-PR bytewise CFB8 adapter while retaining Pumpkin's
/// current outer `BufWriter`, so both benchmark arms have the same downstream
...
impl<W: AsyncWrite + Unpin> AsyncWrite for BytewiseEncryptor<W> {
    fn poll_write(
        ...
        let mut out = [0u8];
        let Ok(out_block): Result<&mut Array<u8, U1>, _> = (&mut out[..]).try_into() else {
            return Poll::Ready(Err(Error::other("invalid CFB8 block size")));
        };
        if self.cipher.encrypt_b2b(&buf[..1], out_block).is_err() {
...
async fn encrypt_buffered(payload: &[u8], fragment_size: usize) -> usize {
    ...
    for fragment in payload.chunks(fragment_size) {
        writer
            .write_all(fragment)
            .await
            .unwrap_or_else(|error| panic!("buffered benchmark write failed: {error}"));
    }
    writer
        .shutdown()
        .await
        .unwrap_or_else(|error| panic!("buffered benchmark shutdown failed: {error}"));
...
fn bench_stream_encryption(c: &mut Criterion) {
    ...
    for size in [64, 1024, 16 * 1024, 64 * 1024, 1024 * 1024] {
        ...
        group.bench_with_input(
            BenchmarkId::new("coalesced_outer_bufwriter", size),
            &payload,
            |b, payload| {
                b.to_async(&runtime).iter(|| async {
    ...
    for fragment_size in [64, 1024, 16 * 1024] {
        ...
        group.bench_with_input(
            BenchmarkId::new("coalesced_outer_bufwriter", fragment_size),
            &fragment_size,
            |b, &fragment_size| {
                b.to_async(&runtime).iter(|| async {
                    black_box(
